...
namespace Form
{
    public partial class MainView : System.Windows.Forms.Form
    {
        ...
        private string path = @"PATHTOYOURDATABASE\ExportDB.xml"; //Ändra till nya pathen när scriptet är granskat och OK
        ...
        static private DataTable GetPatientId(string patientSer)
        {
            DataTable datatable = AriaInterface.Query(@"SELECT 
                                                        ...
                                                        FROM 
	                                                        Patient
                                                        WHERE 		           
                                                            Patient.PatientSer LIKE '" + patientSer + @"'");
            return datatable;
        }
        ...
        private void UpdateList(XDocument document)
        {
            ...
            foreach (var Case in document.Descendants("NewCase")) // Kolla vilka fall uppfyller valda kriterierna och lägg till dem i ListView
            {
